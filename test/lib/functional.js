...
(function() {
  var HARVESTER1_CONFIG, HARVESTER2_CONFIG, LOG_SERVER_CONFIG, LogHarvester, LogServer, TEST_FILES, WEB_SERVER_CONFIG, WebClient, WebServer, _, chai, fpath, fs, i, len, logServer, logging, ref, should, sinon_chai, test, webClient, webServer, winston;
  ...
  ref = require('../../lib/server.js'), LogServer = ref.LogServer, WebServer = ref.WebServer;
  ...
  for (i = 0, len = TEST_FILES.length; i < len; i++) {
    fpath = TEST_FILES[i];
    fs.writeFile(fpath, '');
  }
  ...
  describe('LogServer', function() {
    return it('should have no nodes or streams initially', function() {
      ...
      harvester1.run();
      harvester2.run();
      return describe('Log Server registration', function() {
